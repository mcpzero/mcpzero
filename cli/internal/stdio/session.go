...
func InitializeSession(
	...
	case body := <-respCh:
		if err := json.Unmarshal(body, &resp); err != nil {
			return nil, fmt.Errorf("invalid initialize response (expected JSON-RPC on stdout): %s", truncate(string(body), 160))
		}
		initResponse = append([]byte(nil), body...)
	case err := <-errCh:
	...
	if resp.Error != nil {
		return nil, fmt.Errorf("initialize rejected: %s (code %d)", resp.Error.Message, resp.Error.Code)
	}
