...
| **Gateway** | Edge worker at `https://gw.mcpzero.io` — authenticates API keys, forwards JSON-RPC, writes the activities. |
| **API key** | Credential for calling an endpoint (`Authorization: Bearer <mz_live_api_key>`) from any ai agents or mcp clients. Generated in [Dashboard → API Keys](/app/api-keys). |
